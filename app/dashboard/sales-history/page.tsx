...
import { RecordSaleDialog } from '@/components/dashboard/record-sale-dialog';
...
export default function SalesPage() {
  ...
  const fetchSales = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('sales')
      .select('*, inventory(item_name, purchase_price)')
  ...
  // Financial Summaries
  const totalRevenue = filteredSales.reduce((sum, s) => sum + (Number(s.selling_price) * Number(s.quantity_sold) - (Number(s.discount_amount) || 0)), 0);
  const totalProfit = filteredSales.reduce((sum, s) => {
    const rev = (Number(s.selling_price) * Number(s.quantity_sold)) - (Number(s.discount_amount) || 0);
    const cost = Number(s.inventory?.purchase_price || 0) * Number(s.quantity_sold);
    return sum + (rev - cost);
  }, 0);
  ...
  if (loading && sales.length === 0) {
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin text-emerald-600 mr-2" /> Loading sales history...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* 1. TOP SUMMARY CARDS (Financial Overview) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-slate-200 shadow-sm bg-slate-900 text-white">
          <CardContent className="p-6 flex items-center gap-4">
            <div className="p-3 bg-white/10 rounded-xl"><DollarSign className="h-6 w-6 text-white" /></div>
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase">Filtered Revenue</p>
              <h3 className="text-2xl font-black text-white">K{totalRevenue.toLocaleString()}</h3>
            </div>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-sm bg-emerald-50 border-emerald-100">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-emerald-700 uppercase">Filtered Gross Profit</p>
              <h3 className="text-2xl font-black text-emerald-900">K{totalProfit.toLocaleString()}</h3>
            </div>
          </CardContent>
        </Card>
      </div>

      {/* 2. MAIN SALES HISTORY CARD */}
            ...
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow>
                  ...
                  <TableHead className="text-right font-bold">Purchase Cost</TableHead>
                  <TableHead className="text-right font-bold">Selling Price</TableHead>
                  ...
                  paginatedSales.map((sale) => {
                    ...
                    return (
                      <TableRow key={sale.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <TableCell>
                          <Badge variant="secondary" className="text-[10px] uppercase font-bold bg-slate-100 text-slate-600 border-transparent">
                            {sale.sales_channel || 'Direct'}
                          </Badge>
                        </TableCell>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((page, i, array) => (
                    <div key={page} className="flex items-center">
                      {i > 0 && array[i - 1] !== page - 1 && <span className="text-slate-400 px-1">...</span>}
                      <Button 
